...
from app.services.telegram_deliveries import (
    prepare_morning_delivery,
    send_prepared_morning_delivery,
)
...
def process_morning_delivery_job(delivery_id: int) -> int:
    return process_morning_delivery_send_job(delivery_id)


def process_morning_delivery_preparation_job(delivery_id: int) -> int:
    return run_job_coroutine(_process_morning_delivery_preparation_job(delivery_id))


def process_morning_delivery_send_job(delivery_id: int) -> int:
    return run_job_coroutine(_process_morning_delivery_send_job(delivery_id))


async def _process_morning_delivery_job(delivery_id: int) -> int:
    return await _process_morning_delivery_send_job(delivery_id)


async def _process_morning_delivery_preparation_job(delivery_id: int) -> int:
    ...
    async with SessionLocal() as session:
        try:
            delivery = await prepare_morning_delivery(
                session,
                delivery_id,
            ...
            logger.exception(
                "Telegram delivery preparation job failed",
            ...
            logger.info(
                "Telegram delivery preparation job finished",
                extra={
...
async def _process_morning_delivery_send_job(delivery_id: int) -> int:
    started_at = time.perf_counter()
    status = "failed"
    item_count = 0
    subscriber_id = None
    ...
    async with SessionLocal() as session:
        try:
            ...
            delivery = await send_prepared_morning_delivery(session, delivery_id, sender)
            status = delivery.status
            item_count = len(delivery.items) if "items" in delivery.__dict__ else 0
            subscriber_id = delivery.subscriber_id
            if status == "sent":
